...
If the archive is downloaded for the first time, the package manager [fetches metadata of the package release]([4.2. Fetch information about a package release](../RegistryServerSpecification.md#4.2.-Fetch-information-about-a-package-release)) to obtain the expected checksum.
...
You can reduce the error to a warning by setting the [build]([swift build](../SwiftBuild.md)) option `--resolver-fingerprint-checking` to `warn` (default is `strict`).
...
If the root of the signing certificate is not in the package manager's default trust store, the package author is responsible for telling package users to include the root certificate in their local [trust roots]([Trusted vs. untrusted certificate](../PackageSecurity.md#Trusted-vs.-untrusted-certificate)) directory, otherwise [signature validation]([Validating signed packages](Validating-signed-packages.md)) may fail upon download because the signing certificate is not trusted.

For more information on signed registry packages, see [Publishing to Registry](../UsingSwiftPackageRegistry.md#Publishing-to-Registry).
...
It then performs a series of validations based on user's [security configuration]([Security configuration](../UsingSwiftPackageRegistry.md#Security-configuration)).
...
Package collection publishers may [sign a collection to protect its contents]([Protecting package collections](../PackageCollections.md#Protecting-package-collections)) from tampering. 
...
- The signing certificate must meet all the [requirements]([Requirements on signing certificate](#Requirements-on-signing-certificate)).

Since signing a package collection is optional, the package manager prompts users for confirmation before they can add an [unsigned collection]([Unsigned package collections](../PackageCollections/PackageCollectionAdd.md#Unsigned-package-collections)).
...
On non-Apple platforms, there are no trusted root certificates by default other than those shipped with the [certificate-pinning configuration]([Protecting package collections](../PackageCollections.md#Protecting-package-collections)).
...
When a package manager user tries to add the collection, however, the root certificate must either be preinstalled with the OS (Apple platforms only) or found in the `~/.swiftpm/config/trust-root-certs` directory (all platforms) or shipped with the [certificate-pinning configuration]([Protecting package collections](../PackageCollections.md#Protecting-package-collections)), otherwise the [signature check]([Signed package collections](../PackageCollections/PackageCollectionAdd.md#Signed-package-collections)) fails. Collection publishers should make the DER-encoded root certificate(s) that they use downloadable so that users can adjust their setup if needed.
