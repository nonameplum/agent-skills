...
| `publishedAt` | String | The [ISO 8601]-formatted datetime string of when the package release was published, as recorded by the registry. See related [`originalPublicationTime`]([Appendix B   Package Release Metadata JSON Schema](#Appendix-B---Package-Release-Metadata-JSON-Schema)) in `metadata`. | |
...
##### 4.2.2. Package release metadata standards
 ...
 [Appendix B]([Appendix B   Package Release Metadata JSON Schema](#Appendix-B---Package-Release-Metadata-JSON-Schema))
 defines the JSON schema for package release metadata that
 gets submitted as part of the ["create a package release"]([4.6. Create a package release](#4.6.-Create-a-package-release))
 request. A server MAY allow and/or populate additional metadata by
 expanding the schema. The `metadata` key in the
 ["fetch information about a package release"]([4.2. Fetch information about a package release](#4.2.-Fetch-information-about-a-package-release)) API response
...
as described in [4.2.1]([4.2.1. Package release resources](#4.2.1.-Package-release-resources)).
...
As part of the [package release metadata]([4.2.2. Package release metadata standards](#4.2.2.-Package-release-metadata-standards)) JSON object, the `repositoryURLs` array can be used to specify URLs associated with a package identifier.
...
see [4.6.3]([4.6.3. Synchronous and asynchronous publication](#4.6.3.-Synchronous-and-asynchronous-publication)).
...
A server SHOULD use the `swift package compute-checksum` tool to compute the checksum that's provided in response to a client's subsequent request to [download the source archive]([4.4. Download source archive](#4.4.-Download-source-archive)) for the release.
...
The package release metadata MUST be based on the [JSON schema]([Appendix B   Package Release Metadata JSON Schema](#Appendix-B---Package-Release-Metadata-JSON-Schema)),
as discussed in [4.2.2]([4.2.2. Package release metadata standards](#4.2.2.-Package-release-metadata-standards)).
...
A server MAY make any properties in the [JSON schema]([Appendix B   Package Release Metadata JSON Schema](#Appendix-B---Package-Release-Metadata-JSON-Schema))
...
The `metadata` section of the [create package release request]([4.6. Create a package release](#4.6.-Create-a-package-release))
must be a JSON object of type [`PackageRelease`]([Package Release type](#PackageRelease-type)), as defined in the
JSON schema below.
...
| `organization`    | [Organization]([Organization type](#Organization-type)) | Organization that the author belongs to. | |
