...
collections are accessible to any clients of [libSwiftPM]([Swift Package Manager as a library](../SwiftPMAsALibrary.md)).

`swift package-collection` has the following subcommands:
- [`add`]([swift package-collection add](../PackageCollections/PackageCollectionAdd.md)): Add a new collection
- [`describe`]([swift package-collection describe](../PackageCollections/PackageCollectionDescribe.md)): Get metadata for a collection or a package included in an imported collection
- [`list`]([swift package-collection list](../PackageCollections/PackageCollectionList.md)): List configured collections
- [`refresh`]([swift package-collection refresh](../PackageCollections/PackageCollectionRefresh.md)): Refresh configured collections
- [`remove`]([swift package-collection remove](../PackageCollections/PackageCollectionRemove.md)): Remove a configured collection
- [`search`]([swift package-collection search](../PackageCollections/PackageCollectionSearch.md)): Search for packages by keywords or module names within imported collections
...
All package collections must adhere to the [collection data format]([Input Format](Input-Format.md)) for SwiftPM to be able to consume them.
...
* `identity`: The [identity]([3.6. Package identification](../RegistryServerSpecification.md#3.6.-Package-identification)) of the package if published to a registry. **Optional.**
...
* `signer`: The signer of the package version. **Optional.** Refer to the [documentation]([Package signing](../PackageSecurity.md#Package-signing)) on package signing for details.
...
Package collection generators should include data from the "default" manifest `Package.swift` as well as [version-specific manifest(s)]([Version specific Manifest Selection](../SwiftVersionSpecificPackaging.md#Version-specific-Manifest-Selection))
...
[Version-specific tags]([Version specific tags when resolving remote dependencies](../SwiftVersionSpecificPackaging.md#Version-specific-tags-when-resolving-remote-dependencies)) are not supported by package collections.

### Configuration File

Configuration that pertains to package collections are stored in the file `~/.swiftpm/config/collections.json`. 
It keeps track of user's list of configured collections and preferences such as those set by the `--trust-unsigned` and `--skip-signature-check` flags in the [`package-collection add` command]([swift package-collection add](../PackageCollections/PackageCollectionAdd.md)). 
...
[Sign package collections]([Signed package collections](../PackageSecurity.md#Signed-package-collections)) to establish authenticity and protect their integrity. 
Doing this is optional. 
Users will be prompted for confirmation before they can add an [unsigned collection]([Unsigned package collections](../PackageCollections/PackageCollectionAdd.md#Unsigned-package-collections)).
The signing certificate you use to sign a package collection must meet a list of [requirements]([Requirements on signing certificate](../PackageSecurity.md#Requirements-on-signing-certificate)). If these requirements are not met, the package manager returns an error.

For more details on the security features Package manager implements, see [Package Security](../PackageSecurity.md).
