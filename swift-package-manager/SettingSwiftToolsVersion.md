...
When you create a new package using [swift package init](../Package/PackageInit.md), the minimum version is set automatically to the current version.
...
For more information on providing package manifests for specific Swift versions, see [Packaging based on the version of Swift](../SwiftVersionSpecificPackaging.md).

### Adjusting the tools version.

Edit the `Package.swift` to adjust the version, or use [swift package tools-version](../Package/PackageToolsVersion.md) to report or set the the tools version.
