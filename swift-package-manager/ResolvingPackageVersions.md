...
Run [swift package resolve](../Package/PackageResolve.md) to resolve the dependencies, taking into account the current version constraints in the `Package.swift` manifest and a `Package.resolved` resolved versions file.
...
Run [swift package update](../Package/PackageUpdate.md) to update a package's dependencies to the latest eligible versions, which also updates the `Package.resolved` file.
...
For more information on constraining dependency versions, see [Adding dependencies to a Swift package](../AddingDependencies.md).

If a `Package.resolved` doesn't exist, each user or build system separately resolves dependency versions, only updating when they run [swift package update](../Package/PackageUpdate.md), and new users start with the latest eligible version of each dependency.
If the `Package.resolved` file does exist, any command that requires dependencies (for example, [swift build](../SwiftBuild.md) or [swift run](../SwiftRun.md)) attempts to resolve the versions of dependencies recorded in the file.
